...
int CSocksProxyWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_Connections.InsertColumn(1, TEXT("Source"), LVCFMT_LEFT, 180);
	m_Connections.InsertColumn(2, TEXT("Remote/UDP Relay"), LVCFMT_LEFT, 230);
	m_Connections.InsertColumn(3, TEXT("Download"), LVCFMT_LEFT, 100);
	m_Connections.InsertColumn(4, TEXT("Upload"), LVCFMT_LEFT, 110);
...
LRESULT CSocksProxyWnd::OnProxyConnected(WPARAM wParam, LPARAM lParam)
{
	...
	//保存ctx.
	m_Connections.SetItemData(nIndex, (DWORD_PTR)proxy);
	return 0;
}
