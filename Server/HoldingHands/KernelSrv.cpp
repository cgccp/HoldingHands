...
CKernelSrv::CKernelSrv(CClient *pClient) : CEventHandler(pClient,KNEL)
{
	m_szModulePath[0]        = 0;
...
std::string get_directory(const std::string& full_path) {
	size_t pos = full_path.find_last_of("/\\");  // 查找最后一个斜杠或反斜杠的位置
	if (pos != std::string::npos) {
		return full_path.substr(0, pos);  // 返回目录部分
	}
	return "";  // 如果没有找到斜杠，说明没有目录
}
...
void CKernelSrv::OnGetModuleInfo(const TCHAR*ModuleName){
	...
	if (m_szModulePath[0] == 0)
	{
		...
		if (m_szModulePath[0] == '.') {
			auto s = CString(dir.c_str()) + CString("\\") + m_szModulePath;
			lstrcpy(m_szModulePath, s);
		} else if (m_szModulePath[0] == 0) {
			auto s = CString(dir.c_str()) + CString("\\Modules");
			lstrcpy(m_szModulePath, s);
		}
	}
