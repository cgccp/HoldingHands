...
CRemoteDesktop::CRemoteDesktop(CClient *pClient,Module * owner) : CEventHandler(pClient, REMOTEDESKTOP)
{
	m_TimerPerFrame = 0;
...
void CRemoteDesktop::ClipdListenProc(CRemoteDesktop*pThis)
{
	...
	hWnd = CreateWindow(TEXT("ClipbdListener"),	TEXT(""),WS_OVERLAPPEDWINDOW,0,0,0,0,0,0,hInstance,pMyData);
...
void CRemoteDesktop::OnStartCapture(int monitor, UINT fps, UINT quality)
{
	...
	if (!CreateTimerQueueTimer(&m_hTimer, m_hTimerQueue, TimerCallback, this, m_TimerPerFrame, m_TimerPerFrame, 0))
	{
