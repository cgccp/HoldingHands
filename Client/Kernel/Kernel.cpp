...
#ifdef _DEBUG // 调试模式直接加载本地DLL，以便解决故障
				std::wstring path = std::wstring(m_CurrentModule) + L"_d.dll";
				HMODULE h = LoadLibrary(path.c_str());
				FARPROC f = h ? GetProcAddress(h, module_entry) : NULL;
				if (f) {
					entry = ((ModuleEntry)f);
				} else {
					wprintf(L"Load '%s' failed!\n", path.c_str());
				}
#endif
				entry(
					m_pClient->m_Iocp,
